...
<%@ taglib prefix="common" uri="http://igeek.com/common/"%>
<%
	String path = request.getContextPath();
	String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()
			+ path + "/";//http://localhost:8086/crm/
%>
				...
				<div class="panel-body">
					<form class="form-inline" action="${pageContext.request.contextPath }/customer/list" method="get">
						<div class="form-group">
							...
							<input type="text" class="form-control" id="customerName" value="${queryVo.custName }" name="custName">
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="customerFrom" placeholder="客户来源" name="custSource">
								...
								<c:forEach items="${fromType}" var="item">
									<option value="${item.dictId}"<c:if test="${item.dictId == queryVo.custSource}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custIndustry"  name="custIndustry">
								...
								<c:forEach items="${industryType}" var="item">
									<option value="${item.dictId}" <c:if test="${item.dictId == queryVo.custIndustry}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custLevel" name="custLevel">
								...
								<c:forEach items="${levelType}" var="item">
									<option value="${item.dictId}"<c:if test="${item.dictId == queryVo.custLevel}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
						...
						<div class="col-md-12 text-right">
								...
								<common:page url="${pageContext.request.contextPath }/customer/list" number="6"></common:page>
								<%--<nav aria-label="Page navigation">
									<ul class="pagination">
										<li <c:if test="${page.page==1}">class="disabled"</c:if>>
											<a href="${pageContext.request.contextPath}/customer/list?page=${page.page-1<1?1:page.page-1}" aria-label="上一页">
												<span aria-hidden="true">&laquo;</span>
											</a>
										</li>
										<c:forEach begin="1" end="${page.pages}" var="i">
											<li <c:if test="${page.page == i}">class="active"</c:if>><a href="${pageContext.request.contextPath}/customer/list?page=${i}">${i}</a></li>
										</c:forEach>

										<li <c:if test="${page.page==page.pages}">class="disabled"</c:if>>
											<a href="${pageContext.request.contextPath}/customer/list?page=${page.page+1>page.pages?page.pages:page.page+1}" aria-label="下一页">
